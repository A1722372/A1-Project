-- [[ سكريبت أيهم الأسطوري V12 - النسخة المصلحة والمستقرة بالكامل ]]
...
-- إضافة حافة بنفسجية مضيئة بشكل آمن لحل مشكلة عدم التفعيل
...
-- === [ زر الفتح والإغلاق الجانبي المحدث ] ===
...
for i, name in ipairs(tabs) do
    ...
    btn.Position = UDim2.new(0.05, 0, 0, (i-1) * 46 + 15)
    btn.Text = name 
...
-- === [ دوال مساعدة لإنشاء عناصر الواجهة ] ===
...
local BrightBtn = createButton(MapPage, "تفعيل FullBright (إضاءة كاملة)", 15, Color3.fromRGB(40, 40, 40), function()
    ...
    local b = MapPage:FindFirstChild("FullBrightButton")
    if b then b.BackgroundColor3 = brightActive and Color3.fromRGB(0, 170, 0) or Color3.fromRGB(40, 40, 40) end
    ...
    if data then
        if data.isRainbow then
            rainbowConnection = RunService.RenderStepped:Connect(function()
                local hue = (tick() % 4) / 4
                if FrameStroke and FrameStroke.Parent then FrameStroke.Color = Color3.fromHSV(hue, 1, 1) end
            end)
        else
            if FrameStroke and FrameStroke.Parent then FrameStroke.Color = data.color end
        end
    end
end

local colorContainer = Instance.new("Frame", MapPage)
colorContainer.Size = UDim2.new(0.9, 0, 0, 250) 
colorContainer.Position = UDim2.new(0.05, 0, 0, 95)
colorContainer.BackgroundTransparency = 1

local colorLayout = Instance.new("UIGridLayout", colorContainer)
colorLayout.CellSize = UDim2.new(0, 100, 0, 35) 
...
for idx, mode in ipairs(colors) do
    ...
    cBtn.TextSize = 13 
    cBtn.LayoutOrder = idx
    
    local BtnCorner = Instance.new("UICorner") 
    BtnCorner.CornerRadius = UDim.new(0, 8)
    BtnCorner.Parent = cBtn
    
    if not data.isRainbow then
...
local SpeedInput = createTextBox(PlayerPage, "65", 45) SpeedInput.Text = "65"
local speedActive = false
local SpeedBtn = createButton(PlayerPage, "تفعيل السرعة", 90, Color3.fromRGB(50, 50, 50), function()
    speedActive = not speedActive
    local sb = PlayerPage:FindFirstChild("SpeedButton")
    if sb then sb.BackgroundColor3 = speedActive and Color3.fromRGB(0, 170, 0) or Color3.fromRGB(50, 50, 50) end
...
local JumpInput = createTextBox(PlayerPage, "120", 175) JumpInput.Text = "120"
local jumpActive = false
local JumpBtn = createButton(PlayerPage, "تفعيل القفز", 220, Color3.fromRGB(50, 50, 50), function()
    jumpActive = not jumpActive
    local jb = PlayerPage:FindFirstChild("JumpButton")
    if jb then jb.BackgroundColor3 = jumpActive and Color3.fromRGB(0, 170, 0) or Color3.fromRGB(50, 50, 50) end
...
createButton(PlayerPage, "تفعيل اختراق الجدران (Noclip)", 325, Color3.fromRGB(40, 40, 40), function()
    ...
    local nb = PlayerPage:FindFirstChild("NoclipButton")
    if nb then nb.BackgroundColor3 = noclipActive and Color3.fromRGB(0, 170, 0) or Color3.fromRGB(40, 40, 40) end
...
createButton(PlayerPage, "تفعيل القفز اللانهائي (Inf Jump)", 375, Color3.fromRGB(40, 40, 40), function()
    ...
    local ij = PlayerPage:FindFirstChild("InfJumpButton")
    if ij then ij.BackgroundColor3 = infJumpActive and Color3.fromRGB(0, 170, 0) or Color3.fromRGB(40, 40, 40) end
...
ListContainer.Size = UDim2.new(0.9, 0, 0, 200) 
ListContainer.Position = UDim2.new(0.05, 0, 0, 65)
...
local function updateCPList()
    ...
    for name, cframe in pairs(savedLocations) do
        ...
        GoBtn.Size = UDim2.new(0.75, 0, 1, 0)
        GoBtn.Text = name 
...
createButton(EffectsPage, "تفعيل شظايا الذهب (Gold Particles)", 115, Color3.fromRGB(190, 190, 0), function() giveDirectEffect("Particles", Color3.fromRGB(255, 215, 0)) end)
createButton(EffectsPage, "تفعيل شظايا اللهب (Red Particles)", 165, Color3.fromRGB(190, 0, 0), function() giveDirectEffect("Particles", Color3.fromRGB(255, 0, 0)) end)
createButton(EffectsPage, "إزالة كافة التأثيرات فوراً", 225, Color3.fromRGB(60, 60, 60), function() clearAllEffects() end)

-- === [ زر إغلاق القائمة الرئيسي (X) المحدث ] ===
